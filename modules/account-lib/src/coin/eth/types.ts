...
export class EthTransactionData implements EthLikeTransactionData {
      ...
      new EthereumTx(
        {
          ...
          data: tx.data,
          v: tx.v ?? addHexPrefix((Number(tx.chainId) * 2 + 35).toString(16)),
          r: tx.r,
          s: tx.s,
        },
        { common: common },
  ...
  toJson(): TxData {
    ...
    if (this.tx.v) {
      result.v = addHexPrefix(bufferToHex(this.tx.v));
    } else {
      result.v = addHexPrefix((this.tx.common.chainId() * 2 + 35).toString(16));
    }
